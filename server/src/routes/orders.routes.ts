...
router.get('/', async (req: Request, res: Response) => {
  const items = await prisma.serviceOrder.findMany({ 
    ...
    include: { customer: { select: { name: true } }, device: { select: { brand: true, type: true } } },
...
router.put('/:id', async (req: Request, res: Response) => {
  const item = await prisma.serviceOrder.update({ where: { id: (req.params.id as string) }, data: req.body });
  res.json(item);
});
